// Server-only Enhanced Cache Service with 30-minute intervals and force refresh capability
...
class ServerEnhancedCacheService {
  private cache: Map<string, CacheEntry<any>> = new Map();
  private readonly CACHE_DURATION = 30 * 60 * 1000; // 30 minutes
  private readonly CACHE_METADATA_PATH = path.join(process.cwd(), 'data', 'cache-metadata.json');
  ...
  private async loadCacheMetadata(): Promise<void> {
    try {
      const data = await fs.readFile(this.CACHE_METADATA_PATH, 'utf-8');
      const metadata: CacheMetadata = JSON.parse(data);
      this.lastGlobalRefresh = metadata.lastGlobalRefresh;
      this.refreshHistory = metadata.refreshHistory || [];
      this.stats = { ...this.stats, ...metadata.stats };
    } catch (error) {
      // File doesn't exist or is corrupted, start fresh
      console.log('Cache metadata not found, starting fresh');
    }
  }
  ...
  private async saveCacheMetadata(): Promise<void> {
    try {
      const metadata: CacheMetadata = {
        lastGlobalRefresh: this.lastGlobalRefresh,
        refreshHistory: this.refreshHistory.slice(-100), // Keep last 100 refresh timestamps
        stats: this.stats
      };
      await fs.writeFile(this.CACHE_METADATA_PATH, JSON.stringify(metadata, null, 2));
    } catch (error) {
...
// Export singleton instance - only for server-side use
export const serverEnhancedCacheService = new ServerEnhancedCacheService();
